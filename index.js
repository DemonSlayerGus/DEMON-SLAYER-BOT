...
import makeWASocket, { Browsers, makeCacheableSignalKeyStore, useMultiFileAuthState, fetchLatestBaileysVersion, jidDecode, DisconnectReason, proto } from '@whiskeysockets/baileys';
...
function normalizePhone(input) {
  ...
  if (s.length === 10 && s.startsWith('3')) s = '57' + s; // Colombia
  return s;
}
...
if (methodCodeQR) {
  ...
  opcion = "2";
  if (!phoneNumber) {
    console.log(chalk.bold.redBright(`\nPor favor, Ingrese el número de WhatsApp.\n${chalk.bold.yellowBright("Ejemplo: +57301XXXXXXX")}\n${chalk.bold.magentaBright('---> ')}`));
...
export async function startBot() {
  ...
  if (opcion === "2" && !state.creds.registered) {
    ...
    try {
      ...
      console.log(chalk.bold.white(chalk.bgMagenta(` Código de emparejamiento: `)), chalk.bold.green(code));
      console.log(chalk.yellow(`Ve a WhatsApp > Dispositivos vinculados > Vincular con número`));
    } catch (err) {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reconexion > retriesLimit) {
        log.error(`Demasiados reintentos (${retriesLimit}) — limpiando sesión...`);
        botReady = false;
        reconexion = 0;
        isRestarting = false;
        clearSession();
