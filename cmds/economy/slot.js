...
export default {
  ...
  run: async ({ msg, sock, args, command, text, usedPrefix: prefix }) => {
    ...
    if (remainingTime > 0) {
      return sock.sendMessage(msg.chat, { text: `ꕥ Debes esperar *${formatTime(remainingTime)}* antes de volver a jugar.` }, { quoted: msg })
    }

    if (!args[0] || isNaN(args[0]) || parseInt(args[0]) <= 0) {
      return sock.sendMessage(msg.chat, { text: `《✤》 Por favor, ingresa la cantidad que deseas apostar.` }, { quoted: msg })
    }

    const apuesta = parseInt(args[0])
    if (apuesta < 500) return sock.sendMessage(msg.chat, { text: `《✤》 El mínimo para apostar es de 500 *${currency}*.` }, { quoted: msg })
    ...
    if (user.coins < apuesta) {
      if (user.bank >= apuesta) {
        ...
        return sock.sendMessage(msg.chat, { text: `「✿」 No tienes suficientes *${currency}* ni en tu banco.` }, { quoted: msg })
      }
    }
