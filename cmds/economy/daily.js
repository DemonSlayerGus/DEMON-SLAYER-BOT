  ...
  run: async ({ msg, sock }) => {
    ...
    if (timeSinceLast < oneDay) {
     ...
     return sock.sendMessage(msg.chat, {
        text: `ꕥ Ya has reclamado tu *Daily* de hoy.\n` +
   ...
   if (timeSinceLast > twoDays) {
      ...
      return sock.sendMessage(msg.chat, {
        text: `「✿」 Has reclamado tu recompensa diaria de *${recompensa.toLocaleString()} ${monedas}*! (Día *1*)\n` +
    ...
    await sock.sendMessage(msg.chat, {
      text: `「✿」 Has reclamado tu recompensa diaria de *¥${recompensa.toLocaleString()} ${monedas}* (Día *${user.dailyStreak}*)\n` +
...
function formatRemainingTime(ms) {
  ...
  const m = Math.floor((s % 3600) / 60) // <- aquí estaba "msg" y chocaba
  const seg = s % 60
  const partes = []
