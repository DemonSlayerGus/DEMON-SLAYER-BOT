...
export default {
  ...
  category: 'rpg',
   run: async ({ msg, sock, args, command, text, usedPrefix: prefix }) => {
    ...
    if (remaining > 0) {
      return sock.sendMessage(msg.chat, { text: `✿ Debes esperar *${msToTime(remaining)}* para invocar otro ritual.` }, { quoted: msg })
    }
   ...
   await db.updateChatUser(msg.chat, msg.sender, 'ritualCooldown', user.ritualCooldown)
    ...
    if (roll < 0.05) {
      ...
      return sock.sendMessage(msg.chat, { text: `✿ El ritual salió mal... una maldición te arrebató *${loss.toLocaleString()} ${monedas}*` }, { quoted: msg })
    }
   ...
   await db.updateChatUser(msg.chat, msg.sender, 'coins', user.coins)
    ...
    await sock.sendMessage(msg.chat, { text: msg2 }, { quoted: msg })
